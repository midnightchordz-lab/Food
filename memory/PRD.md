...
- Trial self-heal: `POST /api/trial/activate` recreates the premium trialing subscription (upsert) for any user still inside their trial window (per user-doc `trial_end_date`) even if `has_used_trial` is set. `TRIAL_FEATURES` is a shared constant in `trial.py`.
