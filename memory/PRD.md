...
- OAuth trial: Google (`routes/google_auth.py` `/session`) and Apple (`routes/apple_auth.py` `/apple`) signups now call `auto_start_trial_if_eligible` for NEW users (previously only email/password `/register` and phone OTP did). This was the root cause of Google-signup users having no trial and being wrongly routed to Razorpay ("Payment could not be completed"). Existing OAuth users are healed when the new build calls `/trial/activate` (step-3 auto-start / step-2 self-heal). NOTE: requires a fresh Android build (old APK skips `/activate`) + valid live Razorpay keys in the deploy Secrets panel (prod logged "Razorpay Authentication failed").
