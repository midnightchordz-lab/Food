...
- Deploy blocker fixed (health probe 111): `server.py` used to `raise RuntimeError` at import when `ENVIRONMENT=production` and `CORS_ORIGINS` unset — crashing startup so uvicorn never bound and `/health` returned `111 Connection refused` (deploy fail). Now it logs a warning and continues with an empty origins list (mobile is same-origin/native so CORS isn't enforced). Set `CORS_ORIGINS` in Secrets to enable browser origins. Verified import succeeds under `ENVIRONMENT=production` + empty `CORS_ORIGINS`. Do NOT edit `.gitignore` to track `.env` (platform injects env).
- 10-min reliability fix: `generate-validated` (`routes/quick_recipes.py`) no longer lets slow/failed Gemini image gen block the recipes. Each image is bounded (`asyncio.wait_for` 22s) and the whole image step is capped (40s, `return_exceptions=True`); on timeout it returns the recipes with `image_url=None`. This fixes the intermittent failures/multiple-refreshes (a full run was ~36s, right at the old 60s client timeout). Quick cards now also pass `validatedAiImage`, so any dropped image streams in lazily (cached by the same key) with the verified badge.
- Mood Choice verified images: Discover results use the SAME self-validated pipeline via `POST /api/recipe-image/ai-generate-validated` + `useValidatedRecipeImage` + `RecipeCard validatedAiImage`.
- OAuth trial: Google (`routes/google_auth.py`) and Apple (`routes/apple_auth.py`) new signups now call `auto_start_trial_if_eligible`.
- Trial bulletproof heal (fix-for-good): `POST /api/trial/activate` step 3b — if it would return `trial_used` but the user has ZERO `user_subscriptions` docs (trial never actually delivered), it force-creates a fresh 7-day trialing sub and returns premium=true. A genuinely-used trial always leaves an expired/canceled doc, so this never enables repeat trials. Verified 4/4 (tests/test_trial_bulletproof_heal.py). Combined with step-1 (active sub), step-2 (in-window self-heal), and OAuth signup auto-start, a real new user can never be pushed to payment.
- Pay-now-during-trial: profile (`app/(tabs)/profile.tsx`) shows a "Subscribe now & keep Premium" row + tappable premium card while `premiumInfo.status==='trialing'`, routing to `/paywall?payNow=1`. Paywall (`app/paywall.tsx`) `payNowMode` skips the free-trial `/trial/activate` shortcut and goes straight to Razorpay/RevenueCat checkout, and shows the checkout footer even when already premium (`showPay`). A trialing user has `has_used_trial=true` so `create-subscription` charges immediately for the chosen tenure (monthly=30d / annual=365d) — no second free trial.
- Trial self-heal: `POST /api/trial/activate` recreates the premium trialing subscription (upsert) for any user still inside their trial window (per user-doc `trial_end_date`). `TRIAL_FEATURES` shared constant in `trial.py`.
