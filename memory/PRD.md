...
- Mood Choice verified images: Discover results (`app/(tabs)/index.tsx`) now use the SAME self-validated image pipeline as 10-minute meals. New backend endpoint `POST /api/recipe-image/ai-generate-validated` (`routes/gen_image.py`) generates an AI `image_spec` (must_show/must_not_show/validation_rule via gpt-4o-mini) then calls the shared `_generate_and_validate_image` (Gemini Nano Banana, self-validation verdict + 1 retry, disk cache, per-user/IP daily image cap). Frontend hook `useValidatedRecipeImage` + `RecipeCard validatedAiImage` prop drive the image and the "Verified photo" badge from the returned `validated`. Other RecipeCard surfaces still use the fast single-shot `useAiImage`.
